...
function showData (data) {
	...
	for (var i = 0; i < data.elements.length; i++) {
		if (data.elements[i].tags && data.elements[i].tags.wikipedia) {
			...
			var lang = data.elements[i].tags.wikipedia.split(':')[0];
			var article = data.elements[i].tags.wikipedia.split(':')[1];
			var marker = L.circleMarker([lat, lon]).addTo(markers)
				.bindPopup(''+
					...
					'<a href="http://'+lang+'.wikipedia.org/wiki/'+
					article+'">'+data.elements[i].tags.wikipedia+'</a>'
					);
